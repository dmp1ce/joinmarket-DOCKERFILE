...
RUN apt-get update \
  && apt-get install -y --no-install-recommends ca-certificates=2020* wget=1.20* \
     ...
     libgmp-dev=2:6.1* libltdl-dev=2.4* zlib1g-dev=1:1.2.* libjpeg-dev=1:1.5.* \
     libblas-dev=3.8.* liblapack-dev=3.8.* gfortran=4:8.3.* \
  && apt-get clean \
...
RUN echo "Get Docker install PR" \
 ...
 && git am 669.patch && ./install.sh --docker-install \
